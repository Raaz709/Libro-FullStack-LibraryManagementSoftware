              ...
              {filteredLogs.map((log) => (
                <li key={log.id} className="flex items-start gap-4 px-6 py-4">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-cream">
                    <User className="h-4 w-4 text-camel-dark" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="text-sm font-bold text-ink">{log.action ?? "Activity"}</p>
                        {log.userId && (
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-3">
                      ...
                      <button
                        ...
                        className="text-xs font-semibold text-camel-dark transition-colors hover:text-ink"
                      >
                        View details →
                      </button>
                      <button
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="grid gap-3">
            <div className="grid gap-1.5">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-camel-dark">Action</p>
              <p className="text-sm font-bold text-ink">{viewing?.action ?? "—"}</p>
            </div>
            <div className="grid gap-1.5">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-camel-dark">User</p>
              <p className="text-sm text-ink">{viewing?.userId ? `User #${viewing.userId}` : "System"}</p>
            </div>
            <div className="grid gap-1.5">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-camel-dark">Details</p>
              <pre className="max-h-64 overflow-auto whitespace-pre-wrap rounded-soft bg-cream p-3 text-sm leading-6 text-ink">
                {viewing?.details ?? "—"}
              </pre>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="grid gap-1.5">
                ...
                <p className="text-sm text-ink">{viewing?.ipAddress ?? "—"}</p>
              </div>
              <div className="grid gap-1.5">
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-camel-dark">User agent</p>
                <p className="break-words text-sm text-ink">{viewing?.userAgent ?? "—"}</p>
              </div>
            </div>
          </div>
        </DialogContent>
